...
app.post("/api/auth/register", (req, res) => {
    ...
    if (existingUser) {
        return res.status(400).json({ success: false, message: "A user is already registered" })
    }
        ...
        pm2.list((err, processList) => {
            ...
            const processes = processList.map((process) => {
                return {
                    ...
                    uptime: process.pm2_env.status === "online" ? Date.now() - process.pm2_env.pm_uptime : 0,
    ...
    pm2.connect((err) => {
        ...
        pm2.restart(processId, (err) => {
            ...
            if (err) {
                ...
                return res.status(500).json({ success: false, message: "Error restarting process" })
            }
    ...
    pm2.connect((err) => {
        ...
        pm2.reload(processId, (err) => {
            ...
            if (err) {
                ...
                return res.status(500).json({ success: false, message: "Error reloading process" })
            }
...
app.get("/api/pm2/processes/:id/logs/:type", authenticateToken, (req, res) => {
    ...
    pm2.connect((err) => {
        ...
        pm2.describe(processId, (err, processDesc) => {
            ...
            const process = processDesc[0]
            const logFile = logType === "out" ? process.pm2_env.pm_out_log_path : process.pm2_env.pm_err_log_path
            ...
            try {
                ...
                const maxLines = 1000
                const startIndex = Math.max(0, logLines.length - maxLines)

                res.json(logLines.slice(startIndex))
            } catch (error) {
    ...
    pm2.connect((err) => {
        ...
        pm2.describe(processId, (err, processDesc) => {
            ...
            try {
                ...
                res.status(500).json({ success: false, message: "Error clearing log files" })
            }
        })
    })
})
